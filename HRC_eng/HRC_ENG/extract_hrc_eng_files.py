...
def find_the_last_data_created_date():
    ...
    """
    cmd   = 'ls /data/aschrc1/GENHRC/RAW/HRC_ENG/hrc_rates*fits*  '
    cmd   = cmd + '/data/aschrc1/GENHRC/RAW/HRC_ENG/*/Rates/hrc_rates*fits* >' + zspace
    os.system(cmd)
...
#
    ...
    cmd2 = ' dmmerge infile=@dat.lis"[time=' + str(tstart) + ':' 
    cmd2 = cmd2 + str(tstop) + ',quality=0000000000000000000,mnf=0]" '
...
def append_to_rdb(col, out_rdb, cyear, cmon, cday):
    ...
    oline = cyear + cmon + cday + '\t' + nsamples + '\t' + mean + '\t' 
    oline = oline + sig + '\t' + dmin + '\t' + dmax + '\n'

    with open(out_rdb, 'a') as fo:
        fo.write(oline)
