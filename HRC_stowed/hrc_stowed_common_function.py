...
def run_test():
    ...
    flist = mcf.read_data_file('./ztemp2')
...
#
    ...
    if mc is not None:
        ...
        os.system(cmd)
#
#--- check whether there is a file first
#
    if len(flist) == 0:
        pass
    elif len(flist) == 1:
        cmd = 'cp ' + flist[0] + ' ' + outfits
        os.system(cmd)
    else:
#
...
#
    ...
    mcf.rm_files('zxc99.fits')
    mcf.rm_files('ztemp99.fits')
...
#
    ...
    current = time.strftime('%Y-%m-%dT%H:%M:%S', time.gmtime())
...
def combine_two_fits_files_ascds(fits1, fits2, outfile='ztemp99.fits',  fcol_line=''):
    ...
    cmd2 = cmd2 + '" outfile=' + outfile + '  outBlock="" columnList="' 
    cmd2 = cmd2 + str(fcol_line) + '" clobber="yes"'
...
#-----------------------------------------------------------------------------------------------
#-- select_data_with_logical_mask: for given column name and logical mask, 
#--                                select out data and return data table
#-----------------------------------------------------------------------------------------------
...
def run_arc5gl(operation, start='', stop='', dataset='flight', detector='hrc',\
               level='0', filetype='hrchk',filename='', subdetector=''):
    """
...
#
    if operation == 'browse':
        ...
        cmd_a = cmd0        + ' > ' + zspace2
        cmd_b = cmd1 + cmd2 + ' > ' + zspace2

    elif operation == 'retrieve':
        line  = 'operation = retrieve\n'
        cmd_a = 'cd ' + temp_dir + '; ' + cmd0        + '> ' + zspace2
        cmd_b = 'cd ' + temp_dir + '; ' + cmd1 + cmd2 + '> ' + zspace2
...
def clean_up_fits_list(operation, fname):
    ...
    try:
        out = mcf.read_data_file(fname, remove=1)
    except:
        return []
#
...
def remove_duplicate_from_file(efile):
    ...
    """
    try:
        rdata = mcf.read_data_file(efile)
    except:
        return False
#
