...
#
            try:
                out  = hcf.fitsTableStat(infile, 'time')
                cnt  = out[-1]
            except:
                cnt  = 0
...
def test_data_creation(lyear, lmonth):
    ...
    """
    lmonth = mcf.change_month_format(lmonth)
    lmonth = lmonth.upper()
    chk    = 0
    for sdir in ['hrc_i_115', 'hrc_i_90', 'hrc_s_125', 'hrc_s_125_hi', 'hrc_s_90', 'hrc_s_90_hi']:
        tdir = data_dir + str(lyear) + str(lmonth) + '/' + sdir
        if len(os.listdir(tdir)) > 0:
            chk += 1
    if chk == 0:
        return False

    cmd    = 'ls ' + data_dir + str(lyear) + str(lmonth) + '/*/* > ' + zspace
    os.system(cmd)
    with open(zspace, 'r') as f:
        test = f.read()
    mc = re.search('evt1', test)
...
#
    ...
    ctime = hcf.covertfrom1998sec2(stime)

    hcf.update_header(outfile, 'DATE',     value=ctime,  \
                      comment='Date and time of file creation', exten=0)

    hcf.update_header(outfile, 'DATE-OBS', value=begin,  \
                      comment='TT, with clock correction if clockapp', exten=0)

    hcf.update_header(outfile, 'DATE-END', value=end,    \
                      comment='TT, with clock correction if clockapp', exten=0)

    hcf.update_header(outfile, 'TSTART',   value=tstart, \
                      comment='Data file start time', datatype='Real8', exten=0)

    hcf.update_header(outfile, 'TSTOP',    value=tstop,  \
                      comment='Data file stop time',  datatype='Real8', exten=0)
