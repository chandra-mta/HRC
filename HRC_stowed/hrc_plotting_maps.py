...
#
    try:
        for part in range(1, 4):
            run_map_scripts(hdir, year,  xmin, xmax, ymin, ymax, binsize,\
                            hrc=hrc,xp=xp, xl=xl, part=part,chk=chk)
    except:
        pass
...
#--- convert to png
#
    cmd2 = "echo ''|gs -sDEVICE=ppmraw  -r" + str(size) + "   -q -dNOPAUSE "
    cmd2 = cmd2 + " -sOutputFile=-  ./foo.ps| pnmtopng > " + str(outfile)
    cmd  = cmd1 + cmd2
    bash(cmd, env=ascdsenv)
#
#--- trim the edges (see: https://www.bsu.edu/libraries/wiki/index.php?title=ImageMagick)
#
    cmd = 'mogrify -trim ' + str(outfile)           #---- removing the white space
    os.system(cmd)
#
#--- farther trim "blank" part
#
    if hrc == 'i':
