...
def print_main_page(tyear):
    ...
    output: hrc_stowed_position_study.html/hrc_i_115_main.html / hrc_s_125_hi_main.html 
                                          / hrc_s_125_main.html
    """
    tdir = house_keeping + 'Templates/'
#
#--- update the top page
#
    ifile = tdir + 'hrc_stowed_position_study_template'
...
#
#--- evt1 with peak region enhanced / evt1 with the uniform scale, / evt2 with peak region 
#--- / evt2 with the uniform scale
#
...
def make_table_line(html, hdir, hpart, ypart, slist,  lev, part):
    ...
    input:  html    --- a html page linked from this line
            ...
            part    --- an indicator of whether peak area or standard cropping. 
                        _b for the peak area cropping
    """
...
def update_image_correction_page(year):
    ...
    for k in range(0, len(y_list)):
        ...
        line = line + '<td style="text-align:center"><a href="./Data/Hrc_i_115/hrc_i_115_' 
        line = line + str(y_list[k]) 
    ...
    ifile = house_keeping + 'Templates/hrci_image_correction_template'
    with open(ifile, 'r') as f:
        data = f.read()

    out  = data.replace("#TABLE#", line)
    
    ofile = html_dir + 'hrci_image_correction.html'
...
def read_dead_time_list(dead_list_name, tyear):
    """
    read dead time list 
    input:  dead_list_name      --- a name of dead time list and return dead time 
...
#
        data   = mcf.read_data_file(gain_file1)
        flist1 = []
        for ent in data:
            atemp = re.split('\s+', ent)
            flist1.append(atemp[-1])

        data2 = mcf.read_data_file(gain_file2)
        flist2 = []
        for ent in data2:
            atemp = re.split('\s+', ent)
...
#
        ...
        for ent in missing:
            cmd = 'cp -f  ' + house_keeping + 'Gain_files/' + ent + ' ' 
            cmd = cmd  + html_dir + 'Data_save/Gain_files/.'
            os.system(cmd)
...
#
        for ent in data:
            ...
            line = line + '</th>\n\t<td><a href="./Data_sve/Gain_file/' + atemp[2] 
            line = line + '">' + atemp[2] + '</a></td>\n</tr>\n'
#
