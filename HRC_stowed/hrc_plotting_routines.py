...
#
    ...
    [int_list, slope_list, serr_list, xa_list, ya_list]  \
                    = multi_section_linear_fit(indata[0], \
                            indata[7], xmin, xmax, bpoints, shift=-2000)
    for k in range(0, len(bpoints)+1):
        try:
            plt.plot(xa_list[k], ya_list[k], color='lime', lw =2)
        except:
            pass
    
    add_slope_description(xmin, xmax, ymin, ymax,  slope_list, bpoints)
#
#--- peak width trend ---------------------------------
#
    a2 = plt.subplot(212)
...
#
#--- multi section line fits
#
    #bpoints = [bpoint]
    [int_list, slope_list, serr_list, xa_list, ya_list]  \
                    = multi_section_linear_fit(indata[0], \
                            indata[8], xmin, xmax, bpoints, shift=-2000)
    for k in range(0, len(bpoints)+1):
        try:
            plt.plot(xa_list[k], ya_list[k], color='lime', lw =2)
        except:
...
def add_slope_description(xmin, xmax, ymin, ymax, slope_list, bpoints, tfsize=9):
    """
    ...
    input:  xmin        --- min of x
            ...
            bpoints     --- a list of breaking points        
            tfsize      --- a font size
    ...
    title = title + ' (before ' + str(bpoints[0]) + ')' 
    plt.text(xpos, ypos, title, size=tfsize, weight='bold')

    for k in range(0, len(bpoints)):
        breakp = bpoints[k]
        ypos   = ymin + (0.35 -0.05*k) * ydiff
        title  = 'Slope: %3.3f ' % slope_list[k+1]
...
def plot_panel(data, xmin, xmax, ymin, ymax, entLabels, title, lsize=1, psize=2,\
               fsize=9, ebar=1, tpos = 1, step=4, txpos=''):
    """
    plotting each panel
...
#
        if ebar == 1:
            plt.errorbar(data[pos1], data[pos2], yerr=data[pos3], color=color[k*2],\
                         fmt='o', markersize=psize, label=entLabels[k])
        else:
            plt.plot(data[pos1], data[pos2], color=color[k*2], lw=0, marker='+', \
                     markersize=psize, label=entLabels[k])
#
...
def separate_data_s2hvst(fdata, sdata, sigma, s2hvst):
    ...
    input:  fdata   --- first data
            ...
            s2hvst  --- s2hvst
    output: [fdata5, fdata6, fdata7, fdata8, sdata5, sdata6, sdata7,
            sdata8, sigma5, sigma6, sigma7, sigma8]
    """
#
#--- initialize ---  only positions: 5 - 8 are used
#
    fsave = [[] for i in range(0, 10)]
    ssave = [[] for i in range(0, 10)]
    gsave = [[] for i in range(0, 10)]
#
...
#
    for k in range(0, len(fdata)):
        try:
            ...
            val2 = sdata[k]
            val3 = sigma[k]
            fsave[pos].append(val1)
            ssave[pos].append(val2)
            gsave[pos].append(val3)
        except:
            pass

    return [fsave[5], fsave[6], fsave[7], fsave[8],\
            ssave[5], ssave[6], ssave[7], ssave[8],\
            gsave[5], gsave[6], gsave[7], gsave[8]]
...
#--- initialize 
#
    fsave = [[] for i in range(0, 2)]
    ssave = [[] for i in range(0, 2)]
    gsave = [[] for i in range(0, 2)]
#
...
#
    for k in range(0, len(time)):
        ...
        if time[k] >= 2006 and time[k] <= 2010:
            fsave[1].append(fdata[k])
            ssave[1].append(sdata[k])
            gsave[1].append(sigma[k])
        else:
            fsave[0].append(fdata[k])
            ssave[0].append(sdata[k])
            gsave[0].append(sigma[k])

    return [fsave[0], fsave[1], ssave[0], ssave[1], gsave[0], gsave[1]]
