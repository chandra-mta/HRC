...
def hrc_s_param():
    ...
    return [ampsatfile, ampsfcorfile, badpixfile, degapfile, evtflatfile,\
            hypfile, obsfile, tapfile, gainfile]

#-----------------------------------------------------------------------------------------------
#-- update_gain_selection_file: check whether a new gain file is added and update gain_selection file
#-----------------------------------------------------------------------------------------------
...
def get_gain_file_list():
    ...
    os.system(cmd)
    adata = mcf.read_data_file(zspace, remove=1)
#
#--- get only file name
#
    gdata = []
...
#------------------------------------------------------------
    ...
    def test_find_latest_file(self):
        ...
        infile = house_keeping + '/Gain_files/gain_selection'
        dlist  = mcf.read_data_file(infile)
