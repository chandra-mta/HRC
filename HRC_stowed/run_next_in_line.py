...
hk_col_list = hk_col_list + ['SPTPAST','SPBPAST','SPHVLV','SPHBLV','SCIDPREN',\
                             'S2HVST', 'S2HVLV','QUALITY']
#
#--- set  a few things
#
Null      = 'null'
...
#
            ...
            for m in range(0, len(time_list[0])):
                line = line + str(time_list[0][m]) +'\t' + str(time_list[1][m])  + '\t'  
...
#
            ...
            for m in range(0, len(etime_list[0])):
                line = line + str(etime_list[0][m]) +'\t' + str(etime_list[1][m]) + '\t'  
...
#-----------------------------------------------------------------------------------------------
#-- extract_data_and_combine: extract fits files in the time span(s) 
#---                          and combine all of them to one fits file 
#-----------------------------------------------------------------------------------------------
...
#
    dlist = hcf.run_arc5gl('retrieve', start, stop, dataset='flight',
                           detector='hrc', level='0', filetype='hrchk')
...
def find_time_period():
    ...
    output: syear   --- starting year
            ...
            eyear   --- ending year
            emon    --- ending month
            if also return "Null" if something wrong (e.g. wrong time order)
    """
#
