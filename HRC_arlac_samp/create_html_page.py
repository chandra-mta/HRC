...
hrci_offset = [[0,0], [2,0], [0,2], [-2,0], [0,-2], [2,2], [-2,2], [-2,-2], [2,-2],\
               [4,0], [0,4],[-4,0], [0,-4], [6,0], [0,6], [-6,0], [0,-6], [10,10], \
               [-10,10], [-10,-10], [10,-10]]

hrcs_offset = [[0,0], [2,0], [0,2], [-2,0], [0,-2], [2,2], [-2,2], [-2,-2], [2,-2],\
               [4,0], [-4,0], [4,2], [-4,2], [-4, -2], [4,-2], [6,0], [-6,0], [10,2],\
               [-10,2], [-10,-2],[10,-2]]
...
#
    try:
        ...
        text = text + "<td><a href=\"" + html_top + 'Plots3/' + flist[m] 
        text = text + "_" + str(kpos) + ".html\">" 
...
#
    ...
    out = out + ' The markers and the colors indicate the location of the observation '
    out = out + 'in Y/Z offset sarface. '
    out = out + ' If you hover the mouse over the marker, it will display the distribution '
    out = out + 'of samp data.</p>\n'
...
#
    ...
    out = out + '<div style="padding-left:10px;">\n'
#
...
#
    ...
    out = out + 'fall in. If you click the marker on the location, it will open the trend page '
    out = out + 'of the location. '
    out = out + 'The left plot will open the scaled samp trend and the righ plot will open '
    out = out + 'the PI trend page.</p>'
#
...
#
    ...
    for k in range(0, 21):
        ...
        out = out + '" href="' + hlink + '">\n'
    out = out + '</map>\n'
    out = out + '<img src="./Plots3/hrci_offset_surface.png" alt="hrci yzoffset" '
    out = out + 'usemap="#hrci_samp_map">'
#
...
#
    ...
    for k in range(0, 21):
        ...
        out = out + '" href="' + hlink + '">\n'
    out = out + '</map>\n'
    out = out + '<img src="./Plots3/hrcs_offset_surface.png" alt="hrcs yzoffset" '
    out = out + 'usemap="#hrcs_samp_map">'
#
...
#
    ...
    for k in range(0,21):
        pv = ax.plot(xdata[k], ydata[k], color=point_list[k][0], marker=point_list[k][1],\
                     markersize=7,markeredgecolor='black', markeredgewidth=1.0,\
                     lw = connect, label=offset[k])
...
#--- add legend
#
    legend(loc=1, bbox_to_anchor=(0.95, 1))
#
#--- label axes
#
