...
hrcs_offset = [[0,0], [2,0], [0,2], [-2,0], [0,-2], [2,2], [-2,2], [-2,-2],\
               [2,-2], [4,0], [-4,0], [4,2], [-4,2], [-4, -2], [4,-2], [6,0],\
...
#
    ...
    for col in ['pi', 'samp']:
        ...
        for inst in ['s', 'i']:
            ...
            [time_list, data_list, err_list, cnt_list, bkg_list, obsid_list] \
                        = read_file(infile, inst)

            oline = ''
            for pos in range(0, 21):
                outname      = web_dir + 'Plots3/'  + col + '_' + inst + '_' + str(pos) +  '.html'
                profile_page = web_dir + 'Plots3/Profile_page/' + col + '_' + inst + '_' 
                profile_page = profile_page + str(pos) + '.html'

                [info_list, hlink_list]  = create_info_link(obsid_list[pos], col)

                [fig, lint, lslope, lerr, lint2, lslope2, lerr2] \
                            = plot_single_panel(xmin, xmax, ymin, ymax, time_list[pos],\
                                                data_list[pos], err_list[pos], \
                                                info_list, xname, yname, label='')
    
                make_html_page(fig, outname, col, inst, pos, profile_page)
    
                create_thumb_nail_plots(xmin, xmax, ymin, ymax, time_list[pos],\
                                        data_list[pos], err_list[pos], \
                                        info_list, xname, yname, label='')

                p_name = web_dir + 'Plots3/Thumb_plots/' + col + '_' +  inst 
                p_name = p_name  + '_' + str(pos) +  '_thumb_plot.png'

                cmd    = 'mv out_plot.png ' + p_name
                os.system(cmd)
    
                oline = oline + lint  + '\t' + lslope + '\t' + lerr + '\t'
...
#--- plot data
#
    pv = ax.plot(xdata, ydata, color=colorList[pcolor], \
                 marker=marker, markersize=10, markeredgecolor='black',\
                 markeredgewidth=1.0, lw = connect)
...
#--- plot data
#
    ax.plot(xdata, ydata, color=colorList[pcolor], \
                marker=marker, markersize=3, markeredgecolor='black',\
                markeredgewidth=1.0, lw = connect)
#
#--- plot error bar
#
...
#
    ...
    out = out + '<h3><a href="' + hprofile + '" style=\'color:blue;\'>'
    out = out + 'Open Fitted Distribution Page</a></h3>\n\n'
#
#--- count plots
#
...
#
    ...
    out = out + 'menubar=no,location=no,scrollbars=no,status=no,,width=1000,height=500,resize=no");'
    out = out + '    \n'
    ...
    out = out + '<h3><a href="' + html_top + 'arlac_trends.html"  style=\'color:blue\'>'
