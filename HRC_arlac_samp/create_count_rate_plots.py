...
#
    ...
    for col in ['pi', 'samp']:
        for inst in ['s', 'i']:
            ...
            [time_list, cnt_list, err_list, bkg_list, berr_list, obsid_list] \
                                = read_file(infile, inst, col)

            for pos in range(0, 21):
                outname = web_dir + 'Plots3/Count_rates/' + col +'_' +   inst 
                outname = outname + '_' + str(pos) + '.png'
#
...
#--- main plot
#
                    if len(time_list[pos]) > 3:
                        plot_panel(xmin, xmax, ymin, ymax, time_list[pos], cnt_list[pos],\
                                   err_list[pos], bkg_list[pos], berr_list[pos], xname, yname)
...
#--- plot data
#
    p, = plt.plot(xdata, ydata, color=colorList[3], marker='o', markersize=4,\
                  markeredgecolor='black', markeredgewidth=1.0, lw = connect)
...
#--- plot data
#
    p, = plt.plot(xdata, ydata2, color=colorList[4], marker='o', markersize=4,\
                   markeredgecolor='black', markeredgewidth=1.0, lw = connect)
