...
year_div     = 2012
exclude_list = ['1009']
colorList    = ('blue', 'green', 'red', 'aqua', 'lime', 'fuchsia',\
                'maroon', 'black', 'yellow', 'olive')
...
#
            for sign in ['n_', 'p_']:
                ...
                for step in range(0, 20):
                    ...
                    profile_page = web_dir + 'Plots/Profile_page/' + col +  sign + inst 
                    profile_page = profile_page + '_' + str(step) + '.html'
#
#--- read data
#
...
#
                        [fig, lint, lslope, lerr, lint2, lslope2, lerr2] \
                                  = plot_single_panel(xmin, xmax, ymin, ymax, time_list, \
                                    data_list, err_list,  info_list, xname, yname, label='')
#
#--- make html page for it
#
                        make_html_page(fig, outname, col, inst, sign, step, profile_page)

                        oline = oline + str(step) + '\t' + lint  + '\t' + lslope  + '\t' + lerr 
                        oline = oline + '\t' + '\t' + lint2 + '\t' + lslope2 + '\t' + lerr2 + '\n'
#
#--- thumb nail plot
#
                        create_thumb_nail_plots(xmin, xmax, ymin, ymax, time_list,\
                                                data_list, err_list,  info_list,\
                                                xname, yname, label='')

                        p_name = web_dir + 'Plots/Thumb_plots/' + col + sign + inst 
                        p_name = p_name  + '_' + str(step) + '_thumb_plot.png'
...
#
            ...
            create_thumb_nail_plots(xmin, xmax, ymin, ymax, time_list, data_list,\
                                    err_list,  info_list, xname, yname, label='')

            p_name = web_dir + 'Plots/Thumb_plots/' + col + inst 
            p_name = p_name  + '_' + 'center_thumb_plot.png'
...
def plot_single_panel(xmin, xmax, ymin, ymax, xdata, ydata, yerror, info_list,  xname, yname, \
    ...
    Input:  xmin    --- min x
            ...
            lcolor  ---  color of the fitted line, default = 7 ---> 'black'
                colorList = ('blue', 'red', 'green', 'aqua', 'fuchsia',
                             'lime', 'maroon', 'black', 'olive', 'yellow')
            lsize:      fitted line width, defalut = 1
            resolution-- the resolution of the plot in dpi
            linefit  --- if it is 1, fit a line estimated 
            connect  --- if it is > 0, lsize data point with a line, 
                         the larger the value thinker the line
...
def set_min_max(xdata, ydata, xtime = 0, ybot = -999):
    ...
    if xtime > 0:
        xmin  = 1999
        xmax  = int(time.strtime('%Y', time.gmtime())) + 1
...
#
    ...
    out = out + 'Hover the mouse over  the data point on the plot below to see '
    out = out + 'the information about the data point.</b><br />\n'
...
#
    ...
    out = out + '    msgWindow = open("","displayname","toolbar=no,directories=no,'
    out = out + 'menubar=no,location=no,scrollbars=no,status=no,,'
    out = out + 'width=1000,height=500,resize=no");   \n'
    ...
    out = out + '    msgWindow.document.write("<img src=\'../Indivisual_Plots/"+imgname+"\' '
    out = out + 'border =0 ><p></p></body></html>")   \n'
    ...
    out = out + '<h3><a href="' + html_top + 'hz43.html"  style=\'color:blue\'>'
    out = out + 'Back to Top Page</a></h3>\n'
    ...
    for k in range(0, len(time_sorted)):
        ...
        out = out + '<a href="javascript:WindowOpener(\'' + pname + '\')"><img src="' 
        out = out + ddict[time_sorted[k]] + '" style="width:300px;"></a></td>\n'
