...
#
        if plot_title != '':
                plot_voigt(x, y, center, width, amp, alphaD, alphaL, center,\
                           I, a_back, b_back, plot_title, xname, yname)
...
def plot_voigt(x, y,center, width, amp,  alphaD, alphaL, nu_0, I, a_back, b_back,\
               name='',xname='', yname='', outfile='out.png'):
    """
...
#
    tx  = 0.5 * xmax
    ty  = 0.9 * ymax
    line = 'Amp: ' + str(round(amp,2)) + '   Center: ' + str(round(center,2)) 
    line = line    + '   FWHM: ' + str(round(width,2))
    plt.text(tx, ty, line)
#
#--- actual plotting
#
...
#
    ...
    ty  = 0.9 * ymax
    line = 'Amp: ' + str(round(amp,2)) + '   Center: ' + str(round(center,2)) 
    line = line    + '   Width: ' + str(round(width,2))
    plt.text(tx, ty, line)
#
#--- actual plotting
#
...
#
    ...
    [alphaD, alphaL, nu_0, I, a_back, b_back] = list(popt)
#
#--- compute width: HWHM
#
    c1   = 1.0692
    c2   = 0.86639
    hwhm = c1 * alphaL + numpy.sqrt(c2 * alphaL**2 + 4 * alphaD**2)
#
#--- compute amp
#
    f   = numpy.sqrt(ln2)
...
#
    if len(sys.argv) == 2:
        ...
        pline = '<file name> <type(optional): voigt/gauss> <title(optionanl)> '
        pline = pline + '<x axis name(optional)> <y axis name(optional)>'
        print(pline)
        exit(1)
    else:
