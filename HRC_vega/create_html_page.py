...
#
    ...
    try:
        with open(chk_save, 'r') as f:
            chk = int(f.read().strip())
    except:
        pass
...
def send_notification():
    ...
    text = '\nNew Vega Observations are added to:\n\n'
    text = text + 'https://cxc.cfa.harvard.edu/contrib/cxchrc/HRC_trendings/Vega/vega_vis_montior.html'
    text = text + '\n'
    
    with open(zspace, 'w') as fo:
        fo.write(text)
    
    cmd = 'cat ' + zspace + ' |mailx -s "Subject: New Vega Observation" vkashyap@cfa.harvard.edu'
    os.system(cmd)
    cmd = 'cat ' + zspace + ' |mailx -s "Subject: New Vega Observation" ' + admin
    os.system(cmd)
